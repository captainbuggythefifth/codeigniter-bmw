...
;(function ( $, window, document, undefined ) {
  ...
  defaults = {
    ...
    urlCurl: 'https://site.etoro.com/en/wp-content/themes/etoro/inc/sharrre.php',  //PHP script for google plus...
  ...
  popup = {
    ...
    twitter: function(opt){
	 twitterOpt = opt;
	 twitterWindow = window.open('','twitterWindow','toolbar=0, status=0, width=650, height=360');
	 getShortenUrl((opt.buttons.twitter.url !== '' ? opt.buttons.twitter.url : opt.url));
  ...
  Plugin.prototype.renderer = function () {
    ...
    if(template !== ''){  //if there is a template
      ...
      $(this.element).html(
                            '<div class="box"><a class="count" href="#"><span class="text">' + total + '</span><span class="counterArrow"></span></a>' + 
                            (this.options.title !== '' ? '<a class="share" href="#"><span class="conteiner"><span class="icon"></span><span class="text">' + this.options.title + '</span></span></a>' : '') +
                            '</div>'
                          );
    }
  };
...
function limitText(length, ext, str) {
    
    ext = (str.length > length) ? ext : "";
    return str.substring(0, Math.min(length, str.length)) + ext
}

function getShortenUrl(twitterUrl){
	 
  getAJAXforBitly('https://site.etoro.com/en/wp-content/themes/etoro/javascript/general/get-shorten-url.php?url=' + twitterUrl, 'json', function(data) {
     	fillTwitterWindow(data)
    });
		
}

function fillTwitterWindow(finalUrl){
twitterWindow.location.href =
 "https://twitter.com/intent/tweet?text="+encodeURIComponent( limitText(200,"...",twitterOpt.text))+"&url="+finalUrl+(twitterOpt.buttons.twitter.via !== '' ? '&via='+twitterOpt.buttons.twitter.via : '');
	
}


function getAJAXforBitly(url, type, callback) {
